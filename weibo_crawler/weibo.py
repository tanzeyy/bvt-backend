...
class Weibo(object):
    ...
    def get_one_page(self, page, conn):
        """获取一页的全部微博"""
        try:
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            if type == 'img' or type == 'video':
                return file_dir
            file_path = file_dir + os.sep + str(self.user_id) + '.' + type
            return file_path
        except Exception as e:
            print('Error: ', e)
            traceback.print_exc()
    ...
    def get_pages(self, conn):
        ...
        for page in tqdm(range(1, page_count + 1), desc='Progress'):
            print(u'第%d页' % page)
            for is_end in self.get_one_page(page, conn):
                if is_end:
                    break
...
def run_wb(WeiboID, conn, **kwargs):
    ...
    for page in tqdm(range(1, page_count + 1), desc='Progress'):
        print(u'第%d页' % page)
        wb_generator = wb.get_one_page(page, conn)
        for weibo in wb_generator:
            yield dict(weibo), wb
            
            # created_at = datetime.strptime(weibo['created_at'], "%Y-%m-%d")
            # since_date = datetime.strptime(wb.since_date, "%Y-%m-%d")
            # if created_at < since_date:
            #     if wb.is_pinned_weibo(w):
            #         continue
            #     else:
            #         break
...
if __name__ == '__main__':
    ...
    wb = run_wb(WeiboID, redis_conn, since_date="2020-02-20")
    pass
